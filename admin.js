...
async function checkSession(){
  ...
  const {data:profile,error}=await supabase
    .from('admin_profiles')
    .select('id,display_name,email,can_manage,can_manage_admins,active')
  ...
  if(error||!profile?.active||!profile?.can_manage){
    ...
    message(
      $('loginMessage'),
      'This account is signed in but is not authorised to manage the schedule.',
      true
    );
    return;
  }
  ...
  // Recurring survey generation is now handled automatically
  // by the Supabase scheduled database job.
  await loadAll();
}
...
async function loadAll(){
  ...
  if(currentAdmin?.can_manage_admins){
    requests.push(
      supabase
        .from('admin_profiles')
        .select('id,display_name,email,can_manage,can_manage_admins,active,created_at')
        .order('created_at')
    );
  }
...
const recurrenceLabel=r=>({
  ...
  monthly:'Monthly',
  custom_days:`Every ${r.recurrence_interval} days`
}[r.recurrence]||'One-off');

function render(){
  $('volunteerList').innerHTML=db.volunteers.map(v=>
    record(
      v.name,
      v.email?`Email: ${esc(v.email)}`:'No email address',
      v.active?'Active':'Inactive',
      `volunteer:${v.id}`
    )
  ).join('');

  $('siteList').innerHTML=db.sites.map(s=>{
    const bits=[
      s.address,
      s.latitude!=null&&s.longitude!=null?`${s.latitude}, ${s.longitude}`:null,
      s.three_word_location?`///${s.three_word_location.replace(/^\/+/,'')}`:null
    ].filter(Boolean);

    return record(
      s.name,
      bits.join(' • ')||'No location details',
  ...
  $('teamList').innerHTML=db.teams.map(t=>{
    const mem=db.members
      .filter(m=>m.team_id===t.id)
      .map(m=>volunteerName(m.volunteer_id));
    ...
    return `
      <div class="record">
        ...
        <div>${mem.length} member${mem.length===1?'':'s'}</div>
        <div>${t.active?'Active':'Inactive'}</div>
        <div class="record-actions">
          <button class="secondary edit-team" data-id="${t.id}">Edit</button>
        </div>
      </div>`;
  }).join('');
  ...
  if($('adminList')){
    $('adminList').innerHTML=currentAdmin?.can_manage_admins
      ? db.admins.map(a=>{
          ...
          const access=a.active
            ? (a.can_manage?'Schedule manager':'No schedule access')
            : 'Disabled';
          ...
          return `
            <div class="record">
              <div>
                <strong>${esc(a.display_name||a.email||'Administrator')}</strong><br>
                <small>${esc(a.email||'No email recorded')}</small>
              </div>
              <div>${esc(access+manager)}</div>
              <div>${isSelf?'Your account':''}</div>
              <div class="record-actions">
                <button class="secondary edit-admin" data-id="${a.id}">Permissions</button>
              </div>
            </div>`;
        }).join('')
      : '';
...
function renderRounds(){
    ...
    <div class="round-status-tabs" role="tablist" aria-label="Survey round status">
      ${statuses.map(status=>{
        ...
        return `
          <button
            type="button"
            class="round-status-tab ${activeRoundStatusTab===status?'active':''}"
  ...
  $('roundList').innerHTML=
    ...
    `<div class="round-status-list">${
      filtered.length
        ? filtered.map(r=>{
            const count=db.roundSites
              .filter(x=>x.survey_round_id===r.id).length;

            const actual=
              r.status==='conducted'&&r.conducted_date
            ...
            return `
              <div class="record round-record">
                ...
                <div class="round-record-actions">
                  <label class="round-status-select-label">
                    <span>Status</span>
                    <select
                      class="round-status-select"
                      data-round-status-select="${r.id}">
  ...
  document.querySelectorAll('[data-round-status-tab]').forEach(button=>{
    button.onclick=()=>{
      activeRoundStatusTab=button.dataset.roundStatusTab;
      renderRounds();
    };
  });

  document.querySelectorAll('[data-round-status-select]').forEach(select=>{
    select.onchange=async()=>{
      const roundId=Number(select.dataset.roundStatusSelect);
      ...
      if(newStatus==='conducted'){
        const entered=window.prompt(
          'Enter the actual survey date (YYYY-MM-DD):',
          round.conducted_date||round.survey_date||isoToday()
        );
        ...
        if(!/^\d{4}-\d{2}-\d{2}$/.test(entered)){
          alert('Enter the actual survey date in YYYY-MM-DD format.');
          select.value=round.status;
          return;
        }

        payload.conducted_date=entered;
      }else{
        payload.conducted_date=null;

        if(newStatus==='cancelled'){
          payload.auto_repeat=false;
        }
      }
  ...
  document.querySelectorAll('.edit-round').forEach(button=>{
    button.onclick=()=>openRound(Number(button.dataset.id));
  });
}

function record(title,meta,status,token){
  return `
    <div class="record">
      <div>
      ...
      <div></div>
      <div class="record-actions">
        <button class="secondary edit-record" data-token="${token}">Edit</button>
      </div>
    </div>`;
}

function bindRendered(){
  document.querySelectorAll('.edit-record').forEach(b=>{
    b.onclick=()=>{
      const [type,id]=b.dataset.token.split(':');
      if(type==='volunteer')openVolunteer(Number(id));
      if(type==='site')openSite(Number(id));
      if(type==='round')openRound(Number(id));
    };
  });

  document.querySelectorAll('.edit-team').forEach(
    b=>b.onclick=()=>openTeam(Number(b.dataset.id))
  );

  document.querySelectorAll('.edit-assignment').forEach(
    b=>b.onclick=()=>openAssignment(Number(b.dataset.id))
  );

  document.querySelectorAll('.delete-assignment').forEach(
    b=>b.onclick=()=>deleteAssignment(Number(b.dataset.id))
  );

  document.querySelectorAll('.edit-admin').forEach(
    b=>b.onclick=()=>openAdminPermissions(b.dataset.id)
  );
}
...
function setupTabs(){
  document.querySelectorAll('.tab').forEach(b=>{
    b.onclick=()=>{
      document.querySelectorAll('.tab,.admin-section')
...
function setupButtons(){
  $('addVolunteerBtn').onclick=()=>openVolunteer(null);
  $('addSiteBtn').onclick=()=>openSite(null);
  $('addRoundBtn').onclick=()=>openRound(null);
  $('addTeamBtn').onclick=()=>openTeam(null);
  $('addAssignmentBtn').onclick=()=>openAssignment(null);

  if($('inviteAdminBtn')){
    $('inviteAdminBtn').onclick=openInviteAdmin;
  }
}
...
function setupModal(){
  ...
  $('modalSave').onclick=()=>modalSave&&modalSave();
...
function openModal(title,html,save){
...
function openVolunteer(id){
  const v=id
    ? db.volunteers.find(x=>x.id===id)
    : {name:'',email:'',active:true};

  openModal(
    id?'Edit volunteer':'Add volunteer',
    `<div class="form-grid">
       <label class="field">
         Volunteer name
         <input id="vName" value="${esc(v.name)}">
       </label>

       <label class="field">
         Email address (optional)
         <input id="vEmail" type="email" value="${esc(v.email||'')}">
       </label>
     </div>

     <label class="inline-check">
       <input id="vActive" type="checkbox" ${v.active?'checked':''}>
       Active volunteer
     </label>`,
    async()=>{
      const row={
        name:$('vName').value.trim(),
        email:$('vEmail').value.trim()||null,
        active:$('vActive').checked
      };

      if(!row.name)return alert('Enter a volunteer name.');

      const q=id
        ? supabase.from('volunteers').update(row).eq('id',id)
        : supabase.from('volunteers').insert(row);
...
function openSite(id){
  ...
  openModal(
    id?'Edit survey site':'Add survey site',
    `<div class="form-grid">
       <label class="field">
         Site name
       ...
       <label class="field">
         Address
         <input id="sAddress" value="${esc(s.address||'')}">
       </label>

       <label class="field">
         GPS latitude
         <input id="sLat" inputmode="decimal" value="${s.latitude??''}">
       </label>

       <label class="field">
         GPS longitude
         <input id="sLon" inputmode="decimal" value="${s.longitude??''}">
       </label>
     </div>
     ...
     <label class="field" style="margin-top:14px">
       ...
       <input
         id="sWords"
         value="${esc(s.three_word_location||'')}"
    ...
    async()=>{
      ...
      const words=$('sWords').value.trim().replace(/^\/+/,'');

      if(!name)return alert('Enter a site name.');

      if((lat&&!lon)||(!lat&&lon)){
        return alert('Enter both GPS coordinates or leave both blank.');
      }

      if(words&&words.split('.').length!==3){
        return alert('Use three words separated by full stops.');
      }
      ...
      const row={
        ...
        three_word_location:words||null,
        active:$('sActive').checked
      };

      const q=id
        ? supabase.from('survey_sites').update(row).eq('id',id)
        : supabase.from('survey_sites').insert(row);
...
function openTeam(id){
  ...
  openModal(
    ...
    `<label class="field">
     ...
     <div class="picker-section">
       ...
       <p>Click volunteers to add them, then choose one selected member as coordinator.</p>

       <div class="team-person-grid">
         ${volunteers.map(v=>{
           const mem=memberIds.includes(v.id);
           const coord=t.coordinator_id===v.id;

           return `
             <div
               class="team-person ${mem?'selected-member':''}"
               data-card="${v.id}">

               <button
                 type="button"
                 class="member-toggle ${mem?'selected':''}"
                 data-member="${v.id}">
                 <span class="person-name">${esc(v.name)}</span>
                 <span class="person-state">${mem?'Team member':'Add to team'}</span>
               </button>
     ...
     <label class="inline-check">
       <input id="tActive" type="checkbox" ${t.active?'checked':''}>
       Active team
     </label>`,
    async()=>{
      ...
      ].map(b=>Number(b.dataset.member));

      const coord=document.querySelector('input[name="coord"]:checked');

      if(!name)return alert('Enter a team name.');
      if(!members.length)return alert('Select at least one member.');
      if(!coord)return alert('Choose a coordinator.');

      const coordinator_id=Number(coord.value);

      if(!members.includes(coordinator_id)){
        return alert('Coordinator must be a team member.');
      }
  ...
  document.querySelectorAll('[data-member]').forEach(b=>{
    b.onclick=()=>{
      const card=document.querySelector(`[data-card="${b.dataset.member}"]`);
      const radio=card.querySelector('input[name="coord"]');
      const label=card.querySelector('.coordinator-choice');
      const state=card.querySelector('.person-state');
      const selected=b.classList.toggle('selected');
      ...
      if(!selected&&radio.checked)radio.checked=false;
    };
  });
}
...
function openRound(id){
  ...
  const selected=id
    ? db.roundSites
        .filter(x=>x.survey_round_id===id)
        .map(x=>x.survey_site_id)
    : [];
  ...
  openModal(
    ...
    `<div class="form-grid">
       ...
       <label
         class="field"
         id="conductedDateField"
         style="${r.status==='conducted'?'':'display:none'}">
         Actual survey date
         <input
           id="rConductedDate"
           type="date"
     ...
     <p class="muted" style="margin-top:10px">
       Planned date records when the survey was scheduled.
       Actual survey date records when sampling was really carried out.
     </p>
    ...
    async()=>{
      ...
      const siteIds=[
        ...document.querySelectorAll('[data-rsite].selected')
      ].map(b=>Number(b.dataset.rsite));

      if(!name||!survey_date){
        return alert('Enter a name and planned survey date.');
      }

      if(!siteIds.length){
        return alert('Select at least one survey site.');
      }

      if(status==='conducted'&&!conducted_date){
        return alert('Enter the actual survey date.');
      }
...
function openAssignment(id){
  ...
  const rounds=db.rounds.filter(r=>r.status!=='cancelled');
  const sites=db.sites.filter(s=>s.active);
  const teams=db.teams.filter(t=>t.active);
  const vols=db.volunteers.filter(v=>v.active);

  const selectedTeams=id
    ? db.assignmentTeams
        .filter(x=>x.assignment_id===id)
        .map(x=>x.team_id)
    : [];

  const selectedVols=id
    ? db.assignmentVolunteers
        .filter(x=>x.assignment_id===id)
        .map(x=>x.volunteer_id)
    : [];

  openModal(
    id?'Edit site assignment':'Add site assignment',
    `<div class="form-grid">
       <label class="field">
         Survey round
         <select id="aRound">
           ${rounds.map(r=>`
             <option
               value="${r.id}"
               ${a?.survey_round_id===r.id?'selected':''}>
               ${esc(r.name)} — ${fmtDate(r.survey_date)}
             </option>`).join('')}
         </select>
       </label>
       ...
       <label class="field">
         ...
         <select id="aSite">
           ${sites.map(s=>`
             <option
               value="${s.id}"
               ${a?.survey_site_id===s.id?'selected':''}>
               ${esc(s.name)}
             </option>`).join('')}
         </select>
       </label>
     ...
     <div class="picker-section">
       ...
       <div class="choice-grid">
         ${teams.map(t=>`
           <button
             type="button"
             class="choice ${selectedTeams.includes(t.id)?'selected':''}"
             data-ateam="${t.id}">
             ${esc(t.name)}
     ...
     <div class="picker-section">
       <h3>Individual volunteers</h3>
       <div class="choice-grid">
         ${vols.map(v=>`
           <button
             type="button"
             class="choice ${selectedVols.includes(v.id)?'selected':''}"
             data-avol="${v.id}">
             ${esc(v.name)}
    ...
    async()=>{
      const survey_round_id=Number($('aRound').value);
      const survey_site_id=Number($('aSite').value);

      const teamIds=[
        ...document.querySelectorAll('[data-ateam].selected')
      ].map(b=>Number(b.dataset.ateam));

      const volIds=[
        ...document.querySelectorAll('[data-avol].selected')
      ].map(b=>Number(b.dataset.avol));
      ...
      if(id){
        const {error}=await supabase
          ...
          .update({
            survey_round_id,
            survey_site_id,
            status:(teamIds.length||volIds.length)?'covered':'needed'
          })
          .eq('id',id);
        ...
        const {data,error}=await supabase
          ...
          .upsert(
            ...
            {onConflict:'survey_round_id,survey_site_id'}
          )
          .select('id')
          .single();
  ...
  bindChoices('data-ateam');
  bindChoices('data-avol');
}
...
function openInviteAdmin(){
  ...
  openModal(
    ...
    `<div class="form-grid">
     ...
     <p class="help-text">
       The invited person will automatically receive schedule-management access.
       Admin-management access is optional.
     </p>`,
    async()=>{
...
function openAdminPermissions(id){
  ...
  openModal(
    ...
    `<p>
       <strong>${esc(a.display_name||'Administrator')}</strong><br>
     ...
     <label class="inline-check">
       <input
         id="adminCanManageAdmins"
         type="checkbox"
         ${a.can_manage_admins?'checked':''}
         ${isSelf?'disabled':''}>
     ...
     ${isSelf
       ? '<p class="help-text">For safety, you cannot remove your own Admin Manager rights from this screen.</p>'
       : ''}`,
    async()=>{
      if(isSelf){
        closeModal();
